## 【ZUIKE-OpenWrt】AutoBuild-ImmortalWrt
...
旁路的逻辑应该是单网口模式。根据下面的固件属性可知。单网口默认采取`静态模式`，

用户应当自行修改`vi /etc/config/network`根据自己主路由的网段 自行配置旁路的ip地址。
